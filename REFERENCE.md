...
####
#
# Mode: Single Group / Single Event
# 
...
If you are building a single event site, then there are some configuration options to be aware of - 

* Ensure that the event_configuration in your config file is set to single.
  * The event_configuration property does not change the permalink structure generated by hugo, or the items that are displayed in the menus. You will need to choose the appropriate configuration block for this section from the [example config file above](#site-config-settings).
  * When event_configuration is set to single, it makes a few UI adjustments - 
    * Hides the "Past Events" link on the Events index page. This is not needed as you will only have one event.
    * Replaces the "Upcoming Events" section on the Homepage with an "Upcoming Sessions" section (work-in-progress).
* Remove additional folders from the events content directory, so that Hugo knows to render the information in your event folder (which can be called whatever you like).
  * Ensure that the _index.md file remains in the events subfolder under the content directory. Do not replace this with an index.md (notice the difference in naming convention), as hugo will point the /event/ path to the index.md.

#### Multi Event Sites

* Ensure that the event_configuration in your config file is set to multi.
  * The event_configuration property does not change the permalink structure generated by hugo, or the items that are displayed in the menus. You will need to choose the appropriate configuration block for this section from the [example config file above](#site-config-settings).
  * When event_configuration is set to multi, it makes a few UI adjustments - 
    * Shows the "Past Events" link on the Events index page. This is needed as you will be displaying many events on the site, and over time, some of those will transition to being in the past.
    * Displays the "Upcoming Events" section on the Homepage, and uses the "Home.Events" site configuration property to determine how many upcoming events to show.
* For each new event, create a new event bundle in the events subfolder of the content directory, so that Hugo knows to render the information for each event. The folder name will be the URL slug.
* Ensure that you have a folder called **past** under your events directory, with the configuration provided in the examples in this GitHub repo. If you don't have this, your past events page will not be generated.
* Ensure that the _index.md file remains in the events subfolder under the content directory. Do not replace this with an index.md (notice the difference in naming convention), as hugo will point the /event/ path to the index.md.
...
If you are building a single group site, then there are some configuration options to be aware of - 

* Ensure that the group_configuration in your config file is set to single.
  * The group_configuration property does not change the permalink structure generated by hugo, or the items that are displayed in the menus. You will need to choose the appropriate configuration block for this section from the [example config file above](#site-config-settings).
  * When group_configuration is set to single, it makes a few UI adjustments - 
    * Hides the group section from the sidebar view of any event, even if it is populated in the event's frontmatter.
    * Does not render the groups list view in the /groups/ path.
* Remove additional folders from the groups content directory, so that Hugo knows to render the information in your group folder (which can be called whatever you like).
  * Ensure that the _index.md file remains in the groups subfolder under the content directory. Do not replace this with an index.md (notice the difference in naming convention), as hugo will point the /event/ path to the index.md.

#### Multi Group Sites

* Ensure that the group_configuration in your config file is set to multi.
  * The group_configuration property does not change the permalink structure generated by hugo, or the items that are displayed in the menus. You will need to choose the appropriate configuration block for this section from the [example config file above](#site-config-settings).
  * When group_configuration is set to multi, it makes a few UI adjustments - 
    * Shows the group section from the sidebar view of any event. If you want this to be displayed, then you must populate it in the event's frontmatter.
    * Renders the groups list view in the /groups/ path.
* For each new group, create a new group bundle in the groups subfolder of the content directory, so that Hugo knows to render the information for each group. The folder name will be the URL slug.
* Ensure that the _index.md file remains in the events subfolder under the content directory. Do not replace this with an index.md (notice the difference in naming convention), as hugo will point the /event/ path to the index.md.
